...
def _categorical_psi(reference: pd.Series, current: pd.Series) -> float:
    ...
    eps = 1e-6
    ref_pct = np.array([(reference == c).mean() for c in cats]) + eps
    cur_pct = np.array([(current == c).mean() for c in cats]) + eps
    return float(np.sum((cur_pct - ref_pct) * np.log(cur_pct / ref_pct)))
...
def _detect_numeric(
    ...
    if ref.size == 0 or cur.size == 0:
        return FeatureDrift(name, "numerical", False, 0.0, "ks", 0.0, 1.0)
    ks_stat, p_value = stats.ks_2samp(ref, cur)
    psi = population_stability_index(ref, cur)
    drifted = bool(p_value < ks_pvalue_threshold or psi >= psi_threshold)
    return FeatureDrift(name, "numerical", drifted, psi, "ks", float(ks_stat), float(p_value))
...
def _detect_categorical(
    ...
    cats = sorted(set(reference.dropna().unique()) | set(current.dropna().unique()))
    ref_counts = np.array([(reference == c).sum() for c in cats], dtype=float)
    cur_counts = np.array([(current == c).sum() for c in cats], dtype=float)
    psi = _categorical_psi(reference, current)
    ...
    return FeatureDrift(name, "categorical", drifted, psi, "chi2", float(stat), float(p_value))
